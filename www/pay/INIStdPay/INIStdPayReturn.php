...
<?php
        ...
        try {
            ...
            if (strcmp("0000", $_REQUEST["resultCode"]) == 0) {
                ...
                $signKey 		= "L3UwRHhsRDNSM25kV2cxMUdPK2VKdz09";		//$signKey 		= "SU5JTElURV9UUklQTEVERVNfS0VZU1RS"; 		// 가맹점에 제공된 키(이니라이트키) (가맹점 수정후 고정) !!!절대!! 전문 데이터로 설정금지
                ...
                try {
					...
					if ((strcmp("0000", $resultMap["resultCode"]) == 0) && (strcmp($secureSignature, $resultMap["authSignature"]) == 0) ){	//결제보안 추가 2016-05-18
				       ...
				       ******************************************************************************/
						...
						if($resultMap["goodName"] == '회원 1개월 이용권') {
							$db_length = "1m";
						} else if($resultMap["goodName"] == '선생님 1개월 이용권') {
							$db_length = "1m";
						} else {
							$db_length = "";
						}
						
						$query = "insert into ticket (subject, mb_id, length, tid, moid) values ('{$resultMap["goodName"]}','{$_REQUEST["merchantData"]}','{$db_length}', '{$resultMap["tid"]}', '{$resultMap["MOID"]}')";
...
?>
						...
						<tr>
                            <td align="center">
                                <button onclick="return_home()" style="padding:10px">확 인</button>
								<script type="text/javascript">
									function return_home() {
										location.href = '/act/my.php';
									}
								</script>
                            </td>
                        </tr>
