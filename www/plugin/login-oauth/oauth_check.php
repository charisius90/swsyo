...
if($mb['mb_id']) { // 가입된 회원이면
	...
	if(!$mb['mb_sn']) {
		$mb['mb_sn'] = $mb_sn;
		sql_query(" update {$g5['member_table']} set mb_sn = '{$mb_sn}' where mb_id = '{$mb['mb_id']}' ", false);
	}
	...
	// 동일닉네임이 존재하면 임의 닉네임 발급
	$is_nick = false;
	$tmp_nick = $mb_nick;
	for($i = 0; $i < 999; $i++) {
		if (exist_mb_nick($tmp_nick, $mb_id)) {
			$is_nick = true;
			$tmp_nick = $mb_nick.rand(0,999);
		} else {
			break;
		}
	}
    ...
    $sql = " insert into {$g5['member_table']}
                set mb_id = '{$mb_id}',
					 mb_sn = '{$mb_sn}',
                     mb_password = '".get_encrypt_string($tmp_pw)."',
                     mb_name = '".addslashes($mb_name)."',
                     mb_nick = '".addslashes($mb_nick)."',
    ...
    // 실시간 쪽지 알림 기능
    $sql = " update {$g5['member_table']} set mb_memo_call = '$recv_mb_id', as_memo = 1 where mb_id = '$recv_mb_id' ";
    sql_query($sql, false);
...
<script>
//var slr = opener.document.getElementsByName("slr_url").length; // 이 코드에서 죽는다.
//var slr = opener.document.getElementsByName("slr_url")[0].value;
//var url = "";
...
//alert("오프너 동작");
opener.parent.location = 'https://shallwesay.co.kr/';
window.close();
</script>
